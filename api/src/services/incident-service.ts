...
export class IncidentService {
  async getAll(
    ...
    return db<Incident>("incidents")
      ...
      .whereRaw(
        `"incidents"."id" IN (SELECT "incident_id" FROM "incident_users_view" WHERE LOWER("user_email") = ?)`,
        [email.toLowerCase()],
      )
  ...
  async getById(
    ...
    const item = await db("incidents")
      ...
      .select<Incident>(
        ...
        "departments.name as department_name",
        db.raw(
          `CONCAT(CONCAT("locations"."name", ': '), "locations"."community") as "location_name"`,
        ),
      )
      .first();
    ...
    item.steps = await db("incident_steps")
      .where({ incident_id: item.id })
      .orderBy("order");
    const allActions = await db("actions")
      ...
      .orderBy("due_date")
      .orderBy("id");
    item.investigation = await db("investigations")
      .where({ incident_id: item.id })
      .first();
    ...
    if (item.investigation) {
      item.investigation.investigation_data = JSON.parse(
        item.investigation.investigation_data,
      );
    }
    ...
    item.hazards = await db("incident_hazards")
      ...
      .select(
        "incident_hazards.*",
        "incident_hazard_types.name as incident_hazard_type_name",
      );
    ...
    item.committee_actions = allActions.filter(
      (a) => a.is_committee_task === 1,
    );
  ...
  async getByReportingEmail(email: string): Promise<Incident[]> {
    return db<Incident>("incidents")
      ...
      .innerJoin(
        "incident_users_view",
        "incidents.id",
        "incident_users_view.incident_id",
      )
      .innerJoin(
        "incident_types",
        "incident_types.id",
      ...
      .innerJoin("departments", "departments.code", "incidents.department_code")
      .whereRaw(`LOWER("incident_users_view"."user_email") = ?`, [
        email.toLowerCase(),
      ])
      .select(
        "incidents.*",
